...
This project is a simple Applicant Tracking System (ATS) composed of a frontend React application and a backend Node.js API.

The frontend allows recruiters to submit candidate information through a responsive form.  
The backend receives the data, validates it, and stores candidate records using Prisma.

```mermaid
flowchart LR

A[Recruiter UI<br/>React + Bootstrap] --> B[API Request<br/>POST /candidates]

B --> C[Node.js Backend<br/>Express]

C --> D[Validation<br/>Candidate data + CV file]

D --> E[(Database<br/>Prisma ORM)]
```

Flow

The recruiter fills the candidate form in the React UI.

The frontend sends a POST /candidates request to the backend.

The backend validates the data and uploaded CV file.

Prisma stores the candidate record in the database.
